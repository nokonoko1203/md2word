...
numberingセクションで図番号・表番号の採番形式を指定する。`"sequential"`は連番（図1, 図2, 図3…）、`"chapter"`は章番号付き（図1.1, 図1.2, 図2.1…）になる。章番号は採番の基準レベル（既定ではH1）の番号を基準とし、基準レベルの見出しが変わるとリセットされる。それより下のレベルの変化ではリセットされない。

`base_header`で見出し採番の基準レベルを切り替えられる。

- `"h1"`（既定）: H1から採番する（1 → 1.1 → 1.1.1 → (1) → ①）。
- `"h2"`: H1には番号を付けず（ドキュメントタイトル用）、H2が「1.」になり以降の階層が繰り下がる。図表の章番号もH2基準になる。
- `"none"`: すべての見出しに番号を付けない。`figure_format = "chapter"`の章の区切りは従来どおりH1。

なお、見出しのインデント設定（`indent.headingN_left`など）は`base_header`に関わらずMarkdownの見出しレベルに対応する。番号が付かない見出しには採番由来のインデントが適用されない。

## 対応するMarkdown要素

見出しはH1からH5まで対応しており、自動採番が付く。H1とH2にはWord上で前後の段落間隔を設定し、章や節の区切りが詰まりすぎないようにしている。段落、箇条書き、番号付きリストはネストにも対応する。表には自動で表番号が振られ、画像には図番号が付く。表本文と表ヘッダーの文字サイズは`sizes.table_body`と`sizes.table_header`で変更できる。採番形式は設定で連番・章番号付きを切り替えられる。コードブロック、改ページ、水平線にも対応している。インライン要素としてはテキスト、コード、太字、斜体、リンクを扱える。
